...
WORKDIR /src
COPY ["PdfDocker.vbproj", ""]
RUN dotnet restore "./PdfDocker.vbproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "PdfDocker.vbproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "PdfDocker.vbproj" -c Release -o /app/publish
